...
//I do understand how all of this code works and made sure to comment everything, hopefully that is okay!
{
    ...
    public static int setScoreText; //Static int called setScoretext, used to get the set the score in the lose scene
    ...
    void OnTriggerEnter2D(Collider2D other) //A function that checks to see if the ball enters trigger areas
    {
        if (other.gameObject.tag == "Hor_Wall") //If the ball enters a trigger area with the tag wall their direction will be negative of the value and the audio from source is played
        {
            direction.y = -direction.y;
            source.Play();
        }
        else if (other.gameObject.tag == "Paddle")//Checks if the object it detects has the tag Paddle,
        {
            randomY = Random.Range(0, 2); //A randomY value is determined from a range of 0 to 1
            speed = notPunchedSpeed; //speed is set to notPunchedSpeeds value
        ...
        else if (other.gameObject.tag == "Punch") //If the object has the tage Punch then direction.y is set to 0, direction x is set to a negative value of itself
        ...
        //If the other object has the score wall value
        //Then while direction.y is equal to 0, randomY is set to a random value from 0 to 2
        //If randomY is 0 then direction.y is 1, else its -1
    ...
    private void RandomDirection()
    {
           randomness = Random.value; //A random value is obtained here
            ...
            else if (randomness < 0.5 && direction.y !< -4)//If the random value is less than 0.5 and the direction.y isn't less than 4, an additional value is subtracted to the direction to change its speed
            {
                direction.y = direction.y - 1;
            }
    }
